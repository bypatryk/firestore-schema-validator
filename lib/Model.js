...
class Model {
  ...
  static async runHooks(hooks, data = {}, changedKeys = new Set()) {
  ...
  static async parseData(data, changedKeys) {
    data = await this.runHooks(this._prehooks, data, changedKeys)
    data = await this.validate(data, changedKeys)
    data = await this.runHooks(this._posthooks, data, changedKeys)
